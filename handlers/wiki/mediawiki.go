...
// Package wiki Package mediawiki provides a wrapper for interacting with the Mediawiki API
//
...
func (m *MWApi) Read(pageName string) (*Page, error) {
	query := map[string]string{
		...
		"prop":        "extracts",
		"titles":      pageName,
		"explaintext": "true", // Запрашиваем текст в формате plain text
		"format":      "json", // Указываем формат JSON
	}
...
func (m *MWApi) ReadTextOnly(pageName string) (string, error) {
	query := map[string]string{
		"action":      "query",
		"prop":        "extracts",
		"titles":      pageName,
...
func (m *MWApi) SearchTitles(queryString string) ([]string, error) {
	query := map[string]string{
		"action": "opensearch",
		"search": queryString,
		"limit":  "10",   // Устанавливаем ограничение на количество результатов
		"format": "json", // Указываем формат JSON
	}
	...
	// Определяем структуру для парсинга ответа
	var response []interface{}
	if err := json.Unmarshal(body, &response); err != nil {
		return nil, err
	}

	// Извлекаем заголовки из второго элемента массива в ответе
	titles := []string{}
	if len(response) > 1 {
		for _, item := range response[1].([]interface{}) {
